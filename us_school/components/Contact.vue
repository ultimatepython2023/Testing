<template>
    <div>
		<section class="bg-white position-relative p-0" id="contactModal" style="min-height: 100vh;">
								...
								<form @submit.prevent="contact">
									...
									<!-- Full Name -->
													<div class="mb-3">
														<label for="contactFullName" class="form-label"
															>{{lang.contact_name}} <span class="text-danger">*</span></label
														>
														<input
															type="text"
															v-model="form.name"
													...
													<div class="mb-3">
														<label for="contactEmail" class="form-label"
															>{{lang.contact_email}} <span class="text-danger">*</span></label
														>
														<input
															type="email"
															v-model="form.email"
									...
									<div class="mb-3">
														<label for="contactMessage" class="form-label"
															>{{lang.contact_message}} <span class="text-danger">*</span></label
														>
														<textarea
															class="form-control"
															id="contactMessage"
													...
													<div class="mb-3">
														...
														@verify="onVerify" :loadRecaptchaScript="true" sitekey="6LcfmhAdAAAAAJesgmm8iPyrz5EkEI_vqve3H1nV">
														</vue-recaptcha> -->
														<recaptcha />
														<div v-if="robotMsg != ''" class="alert alert-danger">
															{{robotMsg}}
														</div>
													</div>
									<!-- Send -->
									...
									>
										<div class="d-flex justify-content-center">
											<div class="text text-start mx-2">{{lang.send}}</div>
										</div>
									</button>
								</form>
							</div>
                ...
                axios.post('/contact', this.form)
                    .then(response => {
                        ...
                        if(response.data.result.success){
                            this.form               = {};
                            this.message             = this.lang.message_sent;
                        }else{
                            var string = '<ul>';
                            $.each(response.data.result.errors, function(key, value) {
                                string += '<li> ' + value + '<li>';
                            });
                            string += '</ul>';
							this.errors = string;
						...
						this.inRequest = false;
                    this.errors = 'Ops, Something Went Wrong!';
                });
